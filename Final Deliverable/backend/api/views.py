...
def get_customer_favorite_sandwich(request, customer_key):
    try:
        ...
        # 1. Favorite Sandwich (Most Ordered, excluding sides)
        favorite_query = """
        WITH OrderCounts AS (
            SELECT 
                f.PRODUCTKEY,
            ...
            AND f.PRODUCTKEY NOT IN (18, 19, 20)  -- Exclude sides
        ...
        # 1b. Favorite Side (Most Ordered)
        favorite_side_query = """
        WITH SideOrderCounts AS (
            SELECT 
                f.PRODUCTKEY,
                p.PRODUCTNAME,
                p.PRODUCTCALORIES,
            ...
            JOIN PRODUCT_DIM p ON f.PRODUCTKEY = p.PRODUCTKEY
            WHERE f.CUSTOMERKEY = %s
            AND f.PRODUCTKEY IN (18, 19, 20)  -- Only sides
            GROUP BY f.CUSTOMERKEY, f.PRODUCTKEY, p.PRODUCTNAME, p.PRODUCTCALORIES
        )
        SELECT 
            PRODUCTNAME,
        ...
        FROM FACT_ORDERPROCESSING f
        WHERE f.CUSTOMERKEY = %s
        AND f.PRODUCTKEY NOT IN (18, 19, 20)  -- Exclude sides from total count
        """
        ...
        cursor.execute(favorite_side_query, (customer_key,))
        favorite_side_result = cursor.fetchone()
        ...
        customer_first_name = customer_name_result[0] if customer_name_result else ""

        # Compile the report
        report = {
            'customer_first_name': customer_first_name,
            'favorite_sandwich': {
                'name': favorite_result[0] if favorite_result else None,
                'calories': favorite_result[1] if favorite_result else None,
                'times_ordered': favorite_result[2] if favorite_result else 0
            },
            'favorite_side': {
                'name': favorite_side_result[0] if favorite_side_result else None,
                'calories': favorite_side_result[1] if favorite_side_result else None,
                'times_ordered': favorite_side_result[2] if favorite_side_result else 0
            },
            'order_stats': {
